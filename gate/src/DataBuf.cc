...
/// push in
int DataBuf::PushData( char *buf,int len )
{
...
/// pop out
int DataBuf::PopData(char *buf, int len)
{
	pthread_mutex_lock(&m_Mutex);
...
bool DataBuf::CompareHead( char *buf )
{
	bool find = true;
...
bool DataBuf::PrintErrMsg(int errNu)
{
	return PrintErrMsg("", errNu);
}

bool DataBuf::PrintErrMsg(const char *userMsg, int errNu)
{
	switch(errNu)
	{
