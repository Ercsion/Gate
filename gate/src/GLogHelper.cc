...
GLogHelper::GLogHelper(char *program)
{
    ...
    //FLAGS_logtominservity = true;  //设置将所有log输出到最高级LOG文件中
    FLAGS_v = 3;
    //也可使用系统环境变量GOOGLE_LOG_DIR控制log根目录
    FLAGS_log_dir = "/home/ubuntu/workspace/log";
    google::SetLogDestination(google::INFO,    LOGDIR"/INF_");  //设置 google::INFO 级别的日志存储路径和文件名前缀
    google::SetLogDestination(google::WARNING, LOGDIR"/WAR_");  //设置 google::WARNING 级别的日志存储路径和文件名前缀
    google::SetLogDestination(google::ERROR,   LOGDIR"/ERR_");  //设置 google::ERROR 级别的日志存储路径和文件名前缀
    google::SetLogDestination(google::FATAL,   LOGDIR"/FAT_");  //设置 google::FATAL 级别的日志存储路径和文件名前缀
    FLAGS_logbufsecs = 0;       //缓冲日志输出，默认为30秒，此处改为立即输出
    FLAGS_max_log_size = 100; //最大日志大小为 100MB
    FLAGS_stop_logging_if_full_disk = true;     //当磁盘被写满时，停止日志输出
    google::SetLogFilenameExtension("pc");     //设置文件名扩展，如平台？或其它需要区分的信息
